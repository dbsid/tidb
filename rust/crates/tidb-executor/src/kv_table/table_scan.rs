...
impl TableScanExec {
    ...
    fn next_source_row(&mut self) -> Result<Option<Vec<Datum>>, ExecError> {
        ...
        .map_err(|error| {
            eprintln!(
                "TABLE_SCAN_NEXT_DECODE table_id={} table={} remote={} keep={:?} ranges={:?} error={error:?}",
                self.table.table_id,
                self.table.name,
                self.remote.is_some(),
                self.keep,
            ...
            ExecError::unsupported("table bytes failed to decode")
        })?;
        match next {
                    ...
                    |values: &mut [PartialValue], row: &[Datum]| -> Result<(), ExecError> {
                        for (function, value) in functions.iter().zip(values.iter_mut()) {
                            ...
                            match (value, input) {
                                ...
                                (PartialValue::SumReal(sum), Some(input)) => {
                                    let addend = input.to_f64().map_err(|_| {
                                        ExecError::unsupported("partial SUM requires numeric input")
                                    })?;
                                    *sum = Some(sum.unwrap_or(0.0) + addend.value);
                                }
                                (PartialValue::Extreme { value, is_max }, Some(candidate)) => {
...
impl Executor for TableScanExec {
    fn open(&mut self) -> Result<(), ExecError> {
        ...
        self.remote = self
            ...
            .map_err(|error| {
                eprintln!(
                    "TABLE_SCAN_OPEN_REMOTE_DECODE table_id={} table={} keep={:?} ranges={:?} error={error:?}",
                    self.table.table_id,
                    self.table.name,
                    self.keep,
                    self.handle_ranges,
        ...
        self.cursor = Some(
            self.table
                ...
                .map_err(|error| {
                    eprintln!(
                        "TABLE_SCAN_OPEN_LOCAL_DECODE table_id={} table={} keep={:?} ranges={:?} error={error:?}",
                        self.table.table_id,
                        self.table.name,
                        self.keep,
                        self.handle_ranges,
...
impl crate::table_access::TableAccess for TableScanExec {
    ...
    /// This record cursor walks forward in key order. It can satisfy Go's
    /// ascending `PhysicalTableScan.KeepOrder`, but it does not reverse the
    /// iterator for descending requirements.
